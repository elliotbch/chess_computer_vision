### Jutse pour tester la detection sur les frames sans la videos 
...
def detect_pieces(frame, game_state, corners, original_frame, is_initialized=True, square_size=100):
    """
    ...
    for row in range(8):  # Sur un échiquier 8x8, il y a 7 espaces entre les 8 points
        for col in range(8):
            ...
            # bool_piece = detect_piece_saturation(roi, area_roi)
            # bool_piece = detect_piece_in_square(roi, area_roi, color_square)
...
def pieces_initial(frame, rose_pos, blue_pose, corners, original_frame):
    ...
    """
    #! Hint: detect in which case there is a piece and put a -7 or a 7 in the corresponding cell.
    #!       is_initialized is True ONLY if the 32 pieces have been found !
    #!       -7 and 7 may be false as it is only the initialization state.
    #! /!\   Use the corners from the calibration step as well as the stickers detection to determine the cases and the side of the chessboard
