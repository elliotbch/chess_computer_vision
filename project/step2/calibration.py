...
#*
#* @project ELEN0016-2 - Computer Vision - Student Project - Task 2: Camera Calibration
#* @authors Adrien Martins, Léa Rion and Elliot Bouchy
#* @team: group 9
#* @date 23/10/2024
#* @brief Camera calibration using a chessboard pattern
#*
...
def find_external_corners(corners, board_dimension, square_size=5):
    """
...
def process_corners(int_corners, ext_corners, rose_pos, blue_pos):
    """
    ...
    index_h1, index_h8 = h1_h8(ext_corners, index_a8, index_a1)
...
def process_video(video_path, output_corners_file, output_matrix_file, board_dimension):
    """
        Process the video to perform a complete camera calibration

        :param video_path: the path to the video
        :param output_corners_file: the file to save the corners to
        :param output_matrix_file: the file to save the matrix to
        :param board_dimension: the dimension of the board.
    """
    
    # Define method to find the chessboard corners
    methods = [
        lambda img: img,
    ...
    while cap.isOpened():
        ...
        # Process every 100 frame or the next frame if the flag is set
        if (frame_number == 0) or (((frame_number + 1) % 100) == 0) or process_next_frame:
            
            #*-------------------------------------------------------------*#
            #*--------------- Subtask 1: stickers detection ---------------*#
            #*-------------------------------------------------------------*#
    
            # Get the sticker coordinates
            x_rose, y_rose, x_blue, y_blue, ret_stickers = find_stickers(frame)
            rose_pos, blue_pos = [x_rose, y_rose], [x_blue, y_blue]
            
            # If the stickers are not found, set the flag and process the next frame
            if not ret_stickers:
            ...
            #*-------------------------------------------------------------*#
            #*--- Subtasks 2 & 3: camera calibration & 100th frame info ---*#
            #*-------------------------------------------------------------*#
            ...
            # If the corners are not found, set the flag and process the next frame
            if not ret_corner:
            ...
            external_corners, homography = find_external_corners(corners2, board_dimension)

            # Process the corners to determine the correct order
            final_corners, final_ext_corners = np.array(process_corners(internal_corners, external_corners, rose_pos, blue_pos))

            # Save the corners to the file
            save_corners_to_file(output_corners_file, frame_number, final_ext_corners)
        
            # Draw the polygon (grid) in the correct order and draw the center of the corners on the frame
            ...
            cv2.imwrite(f"./frames/image_{frame_number:06d}.png", frame)
        
            # # Save the homography matrix to the file
            with open(output_matrix_file, 'a') as file:
            
                file.write(f"{frame_number}\n{homography}\n\n")
...
def save_corners_to_file(filename, frame_number, corners):
    """
        Save the corners of the chessboard to a file
        
        :param filename: the name of the file to save the corners to
        :param frame_number: the number of the frame
        :param corners: the corners of the chessboard
    """
    
    # corners will have the coordinates in the order (a1, a8, h8, h1)
    with open(filename, 'a') as file:
        
        # Format each line as: frame_Number (xa1,ya1) (xa8,ya8) (xh8,yh8) (xh1,yh1)
        file.write(f"{frame_number} ({int(corners[0][0])},{int(corners[0][1])}) ({int(corners[1][0])},{int(corners[1][1])}) ({int(corners[2][0])},{int(corners[2][1])}) ({int(corners[3][0])},{int(corners[3][1])})\n")
...
if __name__ == "__main__":
    ...
    video_moving = '../videos/LR_moving.mp4'
    ...
    output_corners_file = os.path.join(script_directory, f"corners_{timestamp}.txt")
    output_matrix_file = os.path.join(script_directory, f"matrix_{timestamp}.txt")
    
    # Video processing
    process_video(video_path, output_corners_file, output_matrix_file, board_dimension)
